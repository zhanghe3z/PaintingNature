...
from models.nets.generator_v2 import Generator
...
class NeRF_background(nn.Module):
    def __init__(self,
                 D=8, W=256,
                 in_channels_xyz=63,
                 skips=[4], use_new_activation=False, tensorf=False, semantic_nc = 184, triplane=False, z_dim = 256, noise=True):
        """
        ...
        self.triplane.apply(weights_init)
...
class NeRF_1p(nn.Module):
    def __init__(self,
                 D=8, W=256,
                 in_channels_xyz=63,
                 skips=[4], use_new_activation=False, tensorf=False, semantic_nc = 184, triplane=False, z_dim = 256, L=16, embed_s=False, noise=True, use_cnn=False):
        """
        D: number of layers for density (sigma) encoder
        W: number of hidden units in each layer
        in_channels_xyz: number of input channels for xyz (3+3*10*2=63 by default)
        ...
        skips: add skip connection in the Dth layer
        """
        super(NeRF_1p, self).__init__()
        self.map_net = StyleVectorizer(256, 8)
        if use_cnn:
            out_dim = 256
        else:
            out_dim = 3
        self.triplane_density = Generator(z_dim, ngf=64, out_dim=out_dim, noise=noise, use_cnn=use_cnn)
        self.triplane_density.apply(weights_init)
        rgb_input =  64  + 127
        # z = torch.randn(1, 256, dtype=torch.float32).cuda()
        self.skips = [4]
        # self.z = nn.Parameter(z)
        self.encoder = Embedding(3, 4)
        if embed_s == False:
            input_ch = 4*6+3
        else:
            input_ch = 4*6+3+32
        ...
        self.transz = nn.Linear(256, 16)
        # input_ch +=16
        input_ch = 0
        input_ch +=16
        self.color_net = nn.ModuleList([nn.Linear(W+input_ch, W)] + [nn.Linear(W, W) if i not in self.skips else nn.Linear(W + input_ch, W) for i in range(D-1)])
        self.color = nn.Linear(W, out_dim)
    ...
    def density(self, x, z, return_feat=False):
        ...
        for triplane in triplanes:
            ...
            plane_coef_point.append(F.grid_sample(xy, coord_xy[:,None], mode='bilinear').view(coord_xy.shape[0],-1,coord_xy.shape[1]))
            plane_coef_point.append(F.grid_sample(yz, coord_yz[:,None], mode='bilinear').view(coord_yz.shape[0],-1,coord_yz.shape[1]))
            plane_coef_point.append(F.grid_sample(xz, coord_xz[:,None], mode='bilinear').view(coord_xz.shape[0],-1,coord_xz.shape[1]))
    ...
    def forward(self, x, z=None, sigma_only=False, render_semantic=False, render_rgb=False, label_feature=None):
        ...
        if render_rgb == True:
            # x = torch.zeros_like(self.encoder(x*3)).to(x)
            # x = self.encoder(x*3)
            z = self.transz(z)
            label_feature = z[:,None].repeat(1, h.shape[1],1)
            if label_feature is not None:
                h = torch.cat((h, label_feature), -1)
            # else:
            # h = torch.cat((h, x), -1)
            ...
            m = nn.Sigmoid()
            pre_color = self.color(h)
            if pre_color.shape[-1] >3:
                color = pre_color
            else:
                color = m(pre_color)
            # color = self.color(h)
...
class NeRF_1p_high(nn.Module):
    def __init__(self,
        ...
        skips: add skip connection in the Dth layer
        """
        super(NeRF_1p_high, self).__init__()
        self.map_net = StyleVectorizer(256, 8)
        if use_cnn:
            out_dim = 256
        else:
            out_dim = 3
        self.triplane_density = Generator_high(z_dim, ngf=64, out_dim=out_dim, noise=noise, use_cnn=use_cnn)
        self.triplane_density.apply(weights_init)
        rgb_input =  64  + 127
        # z = torch.randn(1, 256, dtype=torch.float32).cuda()
        self.skips = [4]
