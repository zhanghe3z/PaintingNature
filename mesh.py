...
class Canvas_view():
    def __init__(self,
        ...
        self.tr.matrix[np.abs(self.tr.matrix) < 0.0000001] = 0
        self.view_changed()
...
def output_3d_photo(verts, colors, faces, Height, Width, hFov, vFov, ref_pose,
                    int_mtx, video_poses, original_H=None, original_W=None,
                    border=None, depth=None, normal_canvas=None, all_canvas=None, debug_path = None):
    ...
    stereos = []
    tops = []; buttoms = []; lefts = []; rights = []; poses = [];
    for tp_id, tp in enumerate(video_poses):
        ...
        poses.append(copy.copy(normal_canvas.tr.matrix.T))
        normal_canvas.translate(-rel_pose[:3,3])
        normal_canvas.rotate(axis=axis, angle=-(angle*180)/np.pi)
        normal_canvas.view_changed()
    ...
    if debug_path is not None:
        clip = ImageSequenceClip(stereos, fps=40)
        clip.write_videofile(debug_path + '.mp4', fps=40)
    return stereos, normal_canvas, poses
